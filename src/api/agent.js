...
const request = {
    get: (url) => axios.get(`${url}&apikey=b0FzW77q5nOdS9y0SA7d1xnWtcf37Z7F&size=50&includeSpellcheck=yes`).then(res => res.data._embedded).catch(err => console.log(err)),
...
const search = {
    ...
    byRadiusAroundCity: (city, radius, page = 0) => request.get(`/events?locale=sv-se&city=${city}&radius=${radius}&unit=km&page=${page}&sort=date,name,asc`).then(res => res.events).catch(err => console.log(err)),
}
